...
- GitHub Actions CI (`.github/workflows/ci.yml`) runs three parallel jobs:
  ...
  - `integration-ci`: matrix over both compose files — runs root `bin/ci`.
...
- Keep compose files minimal — avoid duplicating service config between `compose.yaml` and `compose.split.yaml` (use `extends`).
