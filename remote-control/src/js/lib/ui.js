...
import { PREFERENCE_GROUPS } from "./preferences.js";
...
export class RadioPadUI extends EventEmitter {
  ...
  init() {
    ...
    this._settingsList = document.getElementById("settings-list");
    ...
    for (const eventName of ["ionInput", "ionChange"]) {
      this._settingsList.addEventListener(eventName, () => {
        if (this._settingsSaveState !== "saving") {
          this.setSettingsSaveState("idle");
        }
      });
    }
    ...
    this._settingsSaveButton.addEventListener("click", async () => {
      const settingsMap = Object.fromEntries(
        [...this._settingsList.querySelectorAll("ion-input, ion-select")]
          .map((input) => [input.id?.replace(/^pref-/, ""), input.value])
          .filter(([key]) => key),
      );
      this.setSettingsSaveState("saving");
      await this.emitEvent("settings-save", settingsMap);
    });
  }
  ...
  async toastError(message, { color = "warning" } = {}) {
  ...
  updateAuthState(state) {
    ...
    this._authStatus.innerText = state.enabled
      ? state.signedIn
        ? "Signed in"
        : "Signed out"
      : "Sign-in unavailable";
    ...
    if (this._authHint) {
      this._authHint.innerText = state.enabled
        ? state.signedIn
  ...
  setSettingsSaveState(state = "idle") {
    if (!this._settingsSaveButton) {
      return;
    }

    const nextState = SETTINGS_SAVE_STATES[state] || SETTINGS_SAVE_STATES.idle;
    this._settingsSaveState = state;
    this._settingsSaveButton.innerText = nextState.label;
    this._settingsSaveButton.disabled = nextState.disabled;
    this._settingsSaveButton.setAttribute("aria-busy", nextState.busy);
    this._settingsSaveButton.dataset.saveState = state;
    if (nextState.color) {
      this._settingsSaveButton.color = nextState.color;
    } else {
      this._settingsSaveButton.removeAttribute("color");
    }
  }
  ...
  _renderAuthControls() {
    ...
    const controls = !(state && state.enabled)
      ? []
      : !state.signedIn
        ? [
            this._createAuthButton("Sign in with Google", () => {
              this.emitEvent("auth-sign-in");
            }),
          ]
        : [
            this._createAuthButton(
              "Sign out",
              () => {
                this.emitEvent("auth-sign-out");
              },
              { fill: "outline" },
            ),
            ...(this._copyTokenAvailable
              ? [
                  this._createAuthButton(
                    "Copy API test token",
                    () => {
                      this.emitEvent("auth-copy-token");
                    },
                    { fill: "outline" },
                  ),
                ]
    ...
    this._authActionsItem.hidden = controls.length === 0;
