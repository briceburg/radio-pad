...
class RadioPadApp:
    def __init__(self):
        ...
        self.update_display(title="Plz Connect to Player")
        print("RadioPadApp initialized")

    def reset_stations(self):
        """Reset the internal state of the app."""
        self.current_page_index = 0
        self.current_station_index = None
        self.pages = [{"stations": [], "title": "iCEBURG Radio"}]

    def load_stations(self, stations_list: list):
        ...
        try:
            ...
            self.update_display(title=f"Error: {e.__class__.__name__}")
            traceback.print_exc()
    ...
    def update_display(self, flash=False, title=None):
        ...
        if not title:
            if self.current_station_index is not None:
                title = page["stations"][self.current_station_index].get("name", "?")
            else:
                title = page["title"]
        group[MACROPAD_KEY_COUNT + 1].text = title
        print(f"Updating display: {title}")
    ...
    def handle_player_connection(self):
        """Check for serial connection changes to the Player."""
        if PLAYER.connected and not self.pages[self.current_page_index]["stations"]:
            # If we are connected and have no stations, request them (only once per 3 seconds)
            current_time = time.monotonic()
            if current_time - self._last_station_request_time >= 3:
                self._last_station_request_time = current_time
                self.radio_control("station_list", None)
                self.update_display(title="Player connected!")
        elif not PLAYER.connected and self.pages[self.current_page_index]["stations"]:
            # If we were connected and had stations, clear them.
            print(f"PLAYER: disconnected (usb_cdc not connected), clearing stations.")
            self.reset_stations()
            self.update_display(title="Player disconnected!")
        ...
        if position != self.last_position:
            if self.current_station_index is not None:
                ...
                if position > self.last_position:
                    ...
                    self.switch_page(
                        (self.current_page_index - 1 + len(self.pages))
                        % len(self.pages)
                    )
