...
- Never assume a `/dev/ttyACM*` number. Use `bin/data-port` to discover the CircuitPython CDC2 data interface after device changes have settled.
...
- Use `bin/provision` for idempotent first-time or repeat device provisioning. The approved CircuitPython UF2 is vendored under `vendor/`.
...
- Sync RadioPad code before creating or recreating the Compose player. CircuitPython may reboot and expose a different host device number during the sync.
...
- Compose must not mount storage, install CircuitPython, or sync RadioPad code.
